...
public void OnPluginStart() {
	...
	// ConVars
	sm_fortressblast_action_use = CreateConVar("sm_fortressblast_action_use", "attack3", "Which input a player must send in order to use a powerup.", 0);
	sm_fortressblast_admin_flag = CreateConVar("sm_fortressblast_admin_flag", "z", "Which SourceMod flag should be checked for admin-restricted commands outside of debug mode. Only accepts one flag.", 0);
	sm_fortressblast_blast_buildings = CreateConVar("sm_fortressblast_blast_buildings", "100.0", "How much damage the Blast powerup should deal to buildings. 100.0 = 100%.", 0, true, 0.0);
	sm_fortressblast_bot = CreateConVar("sm_fortressblast_bot", "1", "Whether bots should use powerups.", 0, true, 0.0, true, 1.0);
	sm_fortressblast_bot_min = CreateConVar("sm_fortressblast_bot_min", "2.0", "The minimum amount of time within which a bot will use a powerup.", 0, true, 0.0);
	sm_fortressblast_bot_max = CreateConVar("sm_fortressblast_bot_max", "15.0", "The maximum amount of time within which a bot will use a powerup.", 0, true, 0.0);
	sm_fortressblast_debug = CreateConVar("sm_fortressblast_debug", "0", "Whether debug mode should be enabled, which activates command permission overrides and debug messages in chat.", 0, true, 0.0, true, 1.0);
	sm_fortressblast_dizzy_length = CreateConVar("sm_fortressblast_dizzy_length", "5.0", "How long the dizziness effect of Dizzy Bomb lasts. Useful for preventing vertigo.", 0, true, 0.0);
	sm_fortressblast_dizzy_states = CreateConVar("sm_fortressblast_dizzy_states", "5.0", "How many side-to-side rotations the dizziness effect of Dizzy Bomb has. Useful for preventing vertigo.", 0, true, 0.0);
	sm_fortressblast_drop = CreateConVar("sm_fortressblast_drop", "1", "When players should drop powerups on death. 0 = Never, 1 = If the map has no .json file, 2 = Always.", 0, true, 0.0, true, 2.0);
	sm_fortressblast_drop_rate = CreateConVar("sm_fortressblast_drop_rate", "10.0", "The chance that a player will drop a powerup on death. 100.0 = 100%.", 0, true, 0.0, true, 100.0);
	sm_fortressblast_drop_teams = CreateConVar("sm_fortressblast_drop_teams", "1", "Which team's players will drop powerups on death. 1 = Both, 2 = RED, 3 = BLU.", 0, true, 1.0, true, 3.0);
	sm_fortressblast_event_scream = CreateConVar("sm_fortressblast_event_scream", "1", "When Fortress Blast's Scream Fortress event should be active. 0 = Never, 1 = If TF2 Halloween is active, 2 = Always.", 0, true, 0.0, true, 2.0);
	sm_fortressblast_event_fools = CreateConVar("sm_fortressblast_event_fools", "1", "This is not a ConVar in Fortress Blast. This setting will have no effect on the game.", 0, true, 0.0, true, 2.0);
	sm_fortressblast_event_xmas = CreateConVar("sm_fortressblast_event_xmas", "1", "When Fortress Blast's Smissmas event should be active. 0 = Never, 1 = If TF2 Christmas is active, 2 = Always.", 0, true, 0.0, true, 2.0);
	sm_fortressblast_gifthunt = CreateConVar("sm_fortressblast_gifthunt", "0", "Whether Gift Hunt should be enabled on maps with Gift Hunt .json files.", 0, true, 0.0, true, 1.0);
	sm_fortressblast_gifthunt_countbots = CreateConVar("sm_fortressblast_gifthunt_countbots", "0", "Whether bots should be counted as players when increasing the Gift Hunt goal.", 0, true, 0.0, true, 1.0);
	sm_fortressblast_gifthunt_goal = CreateConVar("sm_fortressblast_gifthunt_goal", "75", "The lowest possible number of gifts required to unlock the objective in Gift Hunt.", 0, true, 1.0);
	sm_fortressblast_gifthunt_increment = CreateConVar("sm_fortressblast_gifthunt_increment", "25", "The amount the Gift Hunt goal should increase with every additional group of players.", 0, true, 0.0);
	sm_fortressblast_gifthunt_multiply = CreateConVar("sm_fortressblast_gifthunt_multiply", "1", "Whether to start multiplying the gifts players collect if they are on the losing team.", 0, true, 0.0, true, 1.0);
	sm_fortressblast_gifthunt_players = CreateConVar("sm_fortressblast_gifthunt_players", "4", "How large each group of players is when increasing the Gift Hunt goal.", 0, true, 1.0);
	sm_fortressblast_gifthunt_rate = CreateConVar("sm_fortressblast_gifthunt_rate", "20.0", "The chance that a gift will appear at an available spawn in Gift Hunt. Client may crash if too high. 100.0 = 100%.", 0, true, 0.0, true, 100.0);
	sm_fortressblast_intro = CreateConVar("sm_fortressblast_intro", "1", "Whether to automatically display the Fortress Blast intro message.", 0, true, 0.0, true, 1.0);
	sm_fortressblast_mannpower = CreateConVar("sm_fortressblast_mannpower", "2", "When Mannpower powerups should be replaced by Fortress Blast powerups. 0 = Never, 1 = If the map does not have a .json file, 2 = Always.", 0, true, 0.0, true, 2.0);
	sm_fortressblast_powerups = CreateConVar("sm_fortressblast_powerups", "-1", "Which powerups can spawn in bitfield form. -1 = None, 1 = Super Bounce, 2 = Shock Absorber, 4 = Super Speed, etc.", 0, true, -1.0);
	sm_fortressblast_powerups_roundstart = CreateConVar("sm_fortressblast_powerups_roundstart", "1", "Whether powerups should appear at the spawn points defined in a .json file when a round starts.", 0, true, 0.0, true, 1.0);
	sm_fortressblast_ultra_spawnchance = CreateConVar("sm_fortressblast_ultra_spawnchance", "0.1", "The chance that the ULTRA POWERUP!! will spawn. Server may crash if too high. 100.0 = 100%.", 0, true, 0.0, true, 100.0);
	
	AutoExecConfig(true, "fortress_blast");
	...
	if (IsValidEdict(entity)) {
		if (id == 0) {
			if (sm_fortressblast_ultra_spawnchance.FloatValue > GetSMRandomFloat(0.0, 99.99)) {
				...
				while (!PowerupIsEnabled(Powerup[entity])) {
					Powerup[entity] = GetSMRandomInt(1, NumberOfPowerups);
				}
			}
		} else {
